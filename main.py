...
async def iss_location():
    ...
    try:
        ...
        data = response.json()
        longitude = float(data['longitude']) # the parsing method may have to change
        latitude = float(data['latitude']) # if the JSON structure changes, probably because of an API change
